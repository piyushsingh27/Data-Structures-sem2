	...
	int arr[MAX];
	  ...
	  layout();
 		do{
  		...
  		system("cls");
		printf("\nDo you want to Delete Element from TRee or Not Press Y for Yes: \t");
		getchar();
		scanf("%c",&choice); 
		...
		{	
			...
			printf("\nWhich element you want to delete :\t");
  			...
  			printf("\nPreorder Traversal Of Tree After Deleting  \n");
			preOrder(root);
			getchar();
		}
	
		system("cls");
		printf("\n\nDo Want to Exit The Program.\nPress 0 (zero) For Exit the Program :\t");
		scanf("%d", &opt);
		}while(opt != 0);
